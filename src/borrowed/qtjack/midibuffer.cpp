...
MidiData MidiBuffer::read(int i, bool *ok) const {
    ...
    return (float)((i >= 0 && i < _size) ? ((MidiData*)(_jackBuffer))[i] : 0.0);
}
...
int MidiBuffer::maximumEventSize() {
    if(!isValid()) {
        return 0;
    }
...
MidiData* MidiBuffer::reserveEvent(int sample, int dataSize) {
    if(!isValid()) {
        return 0;
    }
...
bool MidiBuffer::writeEvent(int sample, MidiData *midiData, int dataSize) {
    if(!isValid()) {
        return false;
    }
