...
AudioSample AudioBuffer::read(int i, bool *ok) const {
    ...
    return (float)((i >= 0 && i < _size) ? ((AudioSample*)(_jackBuffer))[i] : 0.0);
}
...
bool AudioBuffer::addTo(AudioBuffer targetBuffer, float attenuation) const {
    if(!isValid()) {
        return false;
    }
...
void AudioBuffer::multiply(float attenuation) {
    if(!isValid()) {
        return;
    }
